...
- `pipeline.py`中`KalmanFilter`类的初始化参数中，有`process_noise`和`measurement_noise`，分别对应理论中提到的`Q`和`R`，你可以调整它们来测试效果。（这一步应该是必需的）
- `target_selector`中的多目标选择逻辑。模拟器是通过击中的数字和击中点到目标中心的距离来评分。一个视野里可能出现多个目标。当前的选板逻辑是选择数字最大的目标，如果相同就选择confidence较大的，即当前代码第54行：
```python
best = max(candidates, key=lambda d: (d.class_id, d.confidence))
```
这样的好处是可以优先打数字大的目标，坏处是可能会导致目标切换很快，kf拟合不上，你可以探索更优的选板逻辑。
